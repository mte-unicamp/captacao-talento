...
# Project
from bot.forms import (CloseCompanyForm, EditCompanyForm, EditHunterForm,
                       NewCompanyForm, NewHunterForm)
from bot.models import ClosedCompany, Company, Contractor, PostSeller, Reminder, Seller
...
def dashboard(request):
    ...
    categories = {
        ...
        "data": [
            Company.objects.filter(category=i, seller_stage=Global.CLOS).count()
            for i in Global.CATEGORY_LIST
        ],
    }
    closed_months = {
        "regulars": [
            ClosedCompany.objects.filter(date_closed__month=i, contract_type=Global.REGU).count()
            for i in range(1, 8)
        ],
        "startups": [
            ClosedCompany.objects.filter(date_closed__month=i, contract_type=Global.STAR).count()
            for i in range(1, 8)
        ],
    }
...
class NewCompany(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        return render(
            request,
            self.template_name,
            {**common_context, "page_name": self.title, "new_company": True, "form": form,},
        )
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            for l in lists:
                ...
                return HttpResponse(
                    "No Trello list matches the seller. \
...
class CloseCompany(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        return render(
            request,
            self.template_name,
            {**common_context, "page_name": self.title, "close_company": True, "form": form,},
        )
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            lists = Helper.get_nested_objs(
                "boards", os.environ["CONTRACTS_BOARD_ID"], "lists"
            ).json()
            for l in lists:
...
class NewHunter(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        return render(
            request,
            self.template_name,
            {**common_context, "page_name": self.title, "new_hunter": True, "form": form,},
        )
...
class SelectHunter(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        return render(
            request,
            self.template_name,
            {**common_context, "page_name": self.title, "edit_hunter": True, "hunters": hunters,},
        )
...
class EditHunter(View):
    ...
    def get(self, request, pk, *args, **kwargs):
        ...
        form = self.form_class(initial={"email": h.email, "hunter_type": h.hunter_type,})
        return render(
            request,
            self.template_name,
            {
